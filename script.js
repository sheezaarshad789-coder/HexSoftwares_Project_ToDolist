...
if(savedTheme === 'dark') document.getElementById('theme-icon').className = 'fas fa-sun';
    ...
    tasks.forEach((task, index) => {
        ...
        const dateHTML = task.date ? 
            `<span class="due-date ${isOverdue ? 'late' : ''}">
                <i class="far fa-calendar-alt"></i> ${task.date === today ? 'Today' : task.date}
            </span>` : "";
        ...
        li.innerHTML = `
            ...
            <div class="actions">
                <i class="fas ${task.completed ? 'fa-undo' : 'fa-check'}" onclick="toggleTask(${index})" style="color: #10b981;"></i>
                <i class="fas fa-trash" onclick="deleteTask(${index})" style="color: #f87171;"></i>
            </div>
        `;
